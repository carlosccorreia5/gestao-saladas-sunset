...
// Declaração global para as bibliotecas do CDN
declare global {
  interface Window {
    XLSX: any;
    saveAs: any;
...
export default function AdminDashboard() {
  ...
  const [pdfReady, setPdfReady] = useState(false); // NOVO ESTADO
  ...
  // ========== FUNÇÃO CORRIGIDA: Carregar resumo do dia ==========
  ...
  // ========== FUNÇÃO AUXILIAR: Divergências por loja ==========
  const getStoreDivergences = async (date: string, orders: any[], deliveries: any[]): Promise<any[]> => {
    try {
      ...
      for (const order of orders) {
        ...
        if (orderItems) {
          for (const orderItem of orderItems) {
            const saladType = orderItem.salad_types.name;
  ...
  // ========== FUNÇÃO CORRIGIDA: Relatório de perdas ==========
  ...
  // ========== FUNÇÃO CORRIGIDA: Eficiência lucro × perda ==========
  ...
  // ========== FUNÇÕES DE EXPORTAÇÃO PDF ATUALIZADA ==========
  ...
  const exportReport = async (type: 'excel' | 'pdf') => {
    ...
    switch (selectedReport) {
      ...
      case 'profit-loss':
        ...
        fileName = `lucro_perda_${startDate}_a_${endDate}.xlsx`;
        await exportToExcel(profitData, fileName, 'Eficiência');
        break;
      ...
      case 'batch-loss':
        ...
        fileName = `perdas_lote_${startDate}_a_${endDate}.xlsx`;
        await exportToExcel(batchData, fileName, 'Perdas por Lote');
        break;
      ...
      case 'comparative':
        ...
        fileName = `comparativo_${startDate}_a_${endDate}.xlsx`;
        await exportToExcel(compData, fileName, 'Comparativo');
        break;
